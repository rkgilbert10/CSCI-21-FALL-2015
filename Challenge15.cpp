 ...
 * Notes:
 * Sources used: http://www.learncpp.com/cpp-tutorial/74a-returning-values-by-value-reference-and-address/
 */
#include <cassert>
#include <iostream>
#include <string>
 ...
 *        the items array
 */
ShoppingList::ShoppingList (unsigned int newMaxItems) : maxItems(newMaxItems) {
    itemCount = 0;
    items = new string[maxItems];
}
...
string ShoppingList::removeItem (unsigned int index) {
    if (index >= itemCount) {
        ...
        string atIndex = items[index];
        for(unsigned int i = index; i<maxItems; i++) {
            //Replaces what's at index with the one above it. 
            if(i<=itemCount)
...
void ShoppingList::reset (unsigned int newMaxItems) {
    if (newMaxItems < 1) {
        ...
        //Free associated memory of the array, 
         if(items != NULL) {
             delete [] items;
            items = NULL;
        }        
        //and realocate to new size. 
